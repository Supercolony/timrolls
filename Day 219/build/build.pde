...
void setup(){
	...
	H.init(this).use3D(true).background(0);
	smooth(8);
	...
	;
	 ...
	 poly= new HPath().polygon(3).x(capture.x).x(capture.y).size(10).fill(255).anchorAt(H.CENTER);
     // H.add(poly);
     canvas.add(poly);
	...
	pool = new HDrawablePool(5); //Shapes
	// pool.autoAddToStage()
	pool.autoParent(canvas)
		...
		.add ( new HEllipse())
		// .add ( new HRect(5).width(1).height(random(100, 300)), 20 )
	...
	pool4 = new HDrawablePool(40); //Motion objects
	// pool4.autoAddToStage()
	pool4.autoParent(canvas)

		// .add( new HPath().polygon(3) )
		.add( new HEllipse(10) )
		// .add (new HShape("iso.svg").enableStyle(false))
		// .add (new HShape("iso2.svg").enableStyle(false))
		// .add (new HShape("iso3.svg").enableStyle(false))
		// .add (new HShape("iso4.svg").enableStyle(false))
		// .add (new HShape("iso5.svg").enableStyle(false))
		...
		// .add ( new HRect(5).width(1).height(random(100, 300)), 12 )
...
void draw() {
  ...
  for(Hand hand : leap.getHands()){
    ...
    float   sphere_radius    = hand.getSphereRadius();
        ...
        pcolors2.applyColor(poly).alpha(15);   
   	
   	for (HDrawable temp : pool4) {
		HDrawable d = (HDrawable) temp;
		int i = pool4.currentIndex();
		
			pcolors2.applyColor(d).alpha(15);
			// colorField.applyColor(d);
			if(hand_is_left){   
