...
void setup(){
	...
	canvas = (HCanvas) H.add(new HCanvas(P3D).autoClear(false).background(#272930));
	H.add(canvas);
	...
	pool = new HDrawablePool(25); //Shapes
	// pool.autoAddToStage()
			...
			new HCallback() {
				public void run(Object obj) {
					...
					d
						...
						// .strokeCap(ROUND)
						// .strokeWeight(2)
						// .stroke(colors2.getColor(),220)
						// .noFill()
						// .fill(colors2.getColor(),10)
						// .noStroke()
						// .anchorAt(H.CENTER)
						.anchor(random(-200,200),random(-200,200))
