...
void setup() {
	size(displayWidth, displayHeight, OPENGL);
        ...
        H.init(this).background(#131313).autoClear(false);
        H.add(new HRect(width,height).fill(#131313).z(0));
...
for (int i = 0; i < artnum; ++i) { 
		...
		art[i] = loadImage(temp);
                int rand = (int)random(100,250); //art min and max size
...
void draw() {
	H.drawStage();
        ...
        counter++;
        loc.set(random(200, width-200), random(300,height-300));
