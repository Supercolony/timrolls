...
void setup(){
	...
	H.init(this).use3D(true).background(#002fff);
	smooth(8);
	...
	canvas = (HCanvas) H.add(new HCanvas().autoClear(false).fade(3));
	H.add(canvas);
	...
	;
     ...
     canvas.add(poly.x(capture.x).x(capture.y).size(10).noStroke().fill(0).anchorAt(H.CENTER));
